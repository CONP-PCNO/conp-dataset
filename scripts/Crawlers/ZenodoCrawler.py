...
def _create_zenodo_tracker(path, dataset, private_files, restricted):
    ...
    for date in dataset['dates']:
        if date['type']['value'] == 'Last Updated':
...
class ZenodoCrawler(BaseCrawler):
    ...
    def update_if_necessary(self, dataset_description, dataset_dir):
        ...
        # determine if an update is necessary and print out the information if verbose
        is_update_necessary = self.determine_if_update_is_necessary(tracker, dataset_description)
    ...
    def determine_if_update_is_necessary(self, tracker, dataset_description):
        '''
        Determines whether an update of the dataset is necessary based on:
          - the last version DOI if the DOI associated with the dataset was generated by zenodo
          - the updated date present in the Zenodo query JSON response  if the DOI associated
              was NOT generated by Zenodo

        :param tracker: content of the .conp-zenodo-crawler.json file
         :type tracker: dict
        :param dataset_description: the dataset description from the Zenodo JSON response
         :type dataset_description: dict

        :return: True if the dataset needs to be updated, False otherwise
        '''

        # if the DOI associated to the dataset was not generated by zenodo and there is no updated
        # field in the tracker .conp-zenodo-crawler.json, need to update the dataset, otherwise,
        # compare the dates
        if 'zenodo' not in dataset_description['identifier']['identifier']:
            if 'updated' not in tracker['zenodo']:
                return True
            # loop through the dates and compare the last updated date to the one stored in the tracker
            for date in dataset_description['dates']:
                if date['type']['value'] == 'Last Updated' and tracker['zenodo']['updated'] == date['date']:
                    # Same version, no need to update
                    if self.verbose:
                        print("{}, last update {} same as Zenodo last update date, no need to update"
                              .format(dataset_description["title"], date['date']))
                    return False
                else:
                    if self.verbose:
                        print("{}, last update {} different from Zenodo last update date {}, updating"
                              .format(dataset_description["title"], tracker["zenodo"]["updated"], date['date']))
                    return True
        elif tracker["zenodo"]["version"] == dataset_description["latest_version"]:
            # Same version, no need to update
            if self.verbose:
                print("{}, version {} same as Zenodo version DOI, no need to update"
